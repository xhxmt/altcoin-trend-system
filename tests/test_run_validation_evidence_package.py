...
def test_run_comparison_config_persists_stdout_json_when_validator_writes_no_files(tmp_path, monkeypatch):
    ...
    assert Path(result["comparison_readme_path"]).read_text(encoding="utf-8").startswith(
        "# Signal Validation Comparison"
    )


def test_run_comparison_config_returns_insufficient_when_command_fails(tmp_path, monkeypatch):
    for name in (
        "baseline-summary.json",
        "baseline-metadata.json",
        "candidate-summary.json",
    ...
    def fake_run_command(**kwargs):
        return {"name": kwargs["name"], "exit_code": 2, "classification": "failed"}
...
def test_run_evidence_package_integrates_single_comparison_config(tmp_path, monkeypatch):
    comparison_config = {
        "selector": "ultra_high_conviction",
        "change_id": "change-1",
        "change_classification": "non_material",
    ...
    monkeypatch.setattr(_MODULE, "load_traceable_comparison_configs", lambda root: [comparison_config])
    monkeypatch.setattr(
        _MODULE,
        ...
        lambda **kwargs: {
            ...
            "change_id": kwargs["config"]["change_id"],
            "selector": kwargs["config"]["selector"],
...
def test_run_evidence_package_writes_partial_manifest_on_dirty_paths_failure(tmp_path, monkeypatch):
    monkeypatch.setattr(_MODULE, "current_git_sha", lambda cwd: "52e5e9bbc5dd0fc0b3f6738df8bd965e482fb83e")
    monkeypatch.setattr(_MODULE, "dirty_paths", lambda cwd: (_ for _ in ()).throw(RuntimeError("git unavailable")))
    monkeypatch.setattr(_MODULE, "utc_now", lambda: datetime(2026, 4, 25, 12, 15, tzinfo=timezone.utc))
    ...
    assert manifest["overall_status"] == "failed"
    assert manifest["formal_evidence_gate_passed"] is False
    assert "git unavailable" in manifest["error"]
    assert (package_dir / "EVIDENCE_PACKAGE.md").is_file()
...
def test_run_evidence_package_preserves_db_smoke_command_when_junit_missing(tmp_path, monkeypatch):
    ...
    def fake_run_command(**kwargs):
        exit_code = 2 if kwargs["name"] == "db_smoke" else 0
        return {
            "name": kwargs["name"],
            "argv": kwargs["argv"],
            "started_at": "2026-04-25T10:00:00+00:00",
            ...
            "stderr_log": str(tmp_path / f"{kwargs['name']}.stderr.log"),
            "junit_xml": str(kwargs.get("junit_xml")) if kwargs.get("junit_xml") else None,
            "classification": "failed" if exit_code else "passed",
        }
    ...
    manifest = json.loads((package_dir / "run_manifest.json").read_text(encoding="utf-8"))
    assert exit_code == 1
    assert manifest["db_smoke"]["classification"] == "failed"
    assert manifest["db_smoke"]["command"]["exit_code"] == 2
    assert manifest["db_smoke"]["command"]["argv"][-2:] == ["--junitxml", str(package_dir / "db_smoke" / "junit.xml")]
    assert manifest["overall_status"] == "failed"


def test_run_evidence_package_preserves_selector_failure_command(tmp_path, monkeypatch):
    selector_command = {
        "name": "selector_ignition",
        "argv": ["validator", "--signal-family", "ignition"],
        "started_at": "2026-04-25T10:00:00+00:00",
        "finished_at": "2026-04-25T10:00:01+00:00",
        "exit_code": 1,
    ...
    def fake_run_selector_validation(**kwargs):
        raise _MODULE.SelectorValidationError(
            selector=kwargs["selector"],
            command=selector_command,
            reason="validator_failed",
            message="validator failed for selector=ignition",
        )

    monkeypatch.setattr(_MODULE, "run_selector_validation", fake_run_selector_validation)
    ...
    assert exit_code == 1
    assert selector_artifact["artifact_status"] == "failed"
